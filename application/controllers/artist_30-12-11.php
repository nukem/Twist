...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td align="center" valign="top"><table width="639" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td><img src="http://www.soundbooka.com/version1/images/email_footer.jpg" width="639" height="83" border="0" usemap="#Map" alt="'.$this->config->item('base_url').'"/></td>
      </tr>
    </table></td>
  </tr>
</table>
...
<map name="Map" id="Map">
  ...
  <area shape="rect" coords="4,30,636,78" href="'.$this->config->item('base_url').'" />
</map>
</body>
</html>';
...
//die('OK');  
  ...
  public function step1($id=0) {
	...
	if($this->session->userdata('email')!='') $data['other_profiles']=$this->mArtist->other_profile($this->session->userdata('email'));
      //print_r($data);die;
      $this->load->view('template', $data);
    } else {
      $form_data = array(
  ...
  public function word_count($str)
  {
	...
	if ( $cnt<50)
		{
			$this->form_validation->set_message('word_count', 'Add more information so that people know more about you. (Min. 50 words)');
  ...
  public function create_new($id) {
   ...
   private function _doUpload($id) {
	  ...
	  function personal_info($id){
		...
		$updatesql ="UPDATE `artist` a, user u SET a.`first_name` = '{$_POST['first_name']}', a.`last_name` = '{$_POST['last_name']}', a.gender='{$_POST['gender']}', u.dob='{$dob}', u.secret_answer='{$_POST['secret_answer']}', u.secret_question='{$_POST['secret_question']}' ";
