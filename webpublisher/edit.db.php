  ...
  <body>    
    <div>      
      ...
      <div class="infocontainer">            
		  <form action="php/datafeed.db.php?method=adddetails&parent=<?php echo $parent; ?><?php echo isset($event)?"&id=".$event->Id:""; ?>" class="fform" id="fmEdit" method="post">                 
          <label>                    
            <span>                        *Subject:              
            </span>                    
            <div id="calendarcolor">
            </div>
            <input MaxLength="200" class="required safe" id="Subject" name="Subject" style="width:85%;" type="text" value="<?php echo isset($event)?stripslashes($event->Subject):"" ?>" />                     
...
<?php
$locations  = dbq('SELECT id, title, uri FROM wp_structure WHERE type = "location" AND online = 1 ORDER BY position');
?>
            <select id="Location" name="Location" style="width:95%;">
				<option></option>
<?php
foreach($locations as $loc) {
	$selected = '';
	if(isset($event) && $loc['uri'] == $event->Location) {
		$selected = 'selected="selected"';
	}
?>
				<option <?php echo $selected; ?> value="<?php echo $loc['uri']; ?>"><?php echo $loc['title']; ?></option>
<?php
}
?>
			</select>
...
<?php
$Program  = dbq('SELECT id, title, uri FROM wp_structure WHERE type = "submenu" AND parent IN (1524, 1535) AND online = 1 ORDER BY position');
?>
            <select id="Program" name="Program" style="width:95%;">
				<option></option>
<?php
foreach($Program as $pro) {
	$selected = '';
	if(isset($event) && $pro['uri'] == $event->Program) {
		$selected = 'selected="selected"';
	}
?>
				<option <?php echo $selected; ?> value="<?php echo $pro['uri']; ?>"><?php echo $pro['title']; ?></option>
<?php
}
?>
			</select>
...
<?php echo isset($event)?stripslashes($event->Description):""; ?>
</textarea>                
          </label>                
