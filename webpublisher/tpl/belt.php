        ...
        <div class="right-col-padding1"> 
          <div class="width-99pct"> 
            <table class="rec-table"> 
              ...
              <tr> 
                <td colspan="2"><label>Belt icon (.png format, 152 x 24px) &bull;</label><br /> 
                  <input type="file" name="fileId" /></td> 
                <td colspan="2"><label><?= $lang[67] ?></label><br />
              <? if (is_file ($cfg['data'] . $id . "." . $record['extension'])) { ?> 
                  <img src="img/ico-file/<? if (is_file ("img/ico-file/{$record['extension']}.gif")) echo "{$record['extension']}.gif"; else echo "unknown.gif"; ?>" alt="Preview" width="16" height="16" /> <a href="file-preview.php?file=<?=  $id . '.' . $record['extension'] ?>&amp;filename=<?= $record['uri'] . "." . $record['extension'] ?>"><?= $record['uri'] . "." . $record['extension'] ?></a><br />
<?php
if($record['extension'] == 'png') {
?>
<br />	<img src="<?php echo $cfg['data'] . $id . "." . $record['extension']; ?>" />
<?php
}
?>
              <? } else { ?>
