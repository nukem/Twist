            ...
            <table class="rec-table"> 
              <? require ("tpl/inc/record.php"); ?> 
				...
				<tr> 
					...
					<td colspan="2">
						<label>Email form results to:</label><br />
						<input type="input" name="email" value="<?php echo show_value('email'); ?>" class="width-100pct textfield" />
					</td>
					<td>
						<label>Save form results to database:</label><br />
						<select name="store_db" class="width-100pct textfield">
							<option value="0" <?php if(show_value('store_db') == 0) { echo 'selected="selected"';} ?>)>No</option>
							<option value="1" <?php if(show_value('store_db') == 1) { echo 'selected="selected"';} ?>)>Yes</option>
						</select>
					</td>
					<td>
						<label>Add form results to leads database</label><br />
						<select name="add_lead" class="width-100pct textfield">
							<option value="0" <?php if(show_value('add_lead') == 0) { echo 'selected="selected"';} ?>)>No</option>
							<option value="1" <?php if(show_value('add_lead') == 1) { echo 'selected="selected"';} ?>)>Yes</option>
						</select>
					</td>
				</tr>
				<tr>
...
<p style="margin-top: 0px;">There is not need to add these items as fields below.</p>
						<input type="button" onclick="return addFormElement()" value="Add New Item" />
...
?>
							<li class="form-item">
									...
									<label class="form-label">
										Name / Label<br />
										<input type="text" name="label" value="<?php echo $f['label']; ?>" class="width-100pct textfield" />
									</label>

									<label class="form-type">
										Item Type<br />
										<select name="type" class="width-100pct textfield">
<?php
	foreach($form_types as $type => $name) {
		$selected = '';
		if($f['type'] == $type) {
...
?>
										...
										<label class="form-required">Is this field required?<br />
											<select name="required" class="width-100pct textfield">
												<option value="0" <?php if($f['required'] == 0) { echo 'selected="selected"';} ?>)>No</option>
												<option value="1" <?php if($f['required'] == 1) { echo 'selected="selected"';} ?>)>Yes</option>
											</select>
										</label>

										<label class="form-email">Valid email address required?<br />
											<select name="valid_email" class="width-100pct textfield">
												<option value="0" <?php if($f['valid_email'] == 0) { echo 'selected="selected"';} ?>)>No</option>
												<option value="1" <?php if($f['valid_email'] == 1) { echo 'selected="selected"';} ?>)>Yes</option>
											</select>
										</label>

									<label class="form-values">
										Drop Down / Checkbox / Radio Button Options<br />
										(Seperate each value with a comma. e.g. Example one,Sample Item 2,Three)<br />
										<input type="text" name="values" value="<?php echo $f['values']; ?>" class="width-100pct textfield" />
									</label>

									<div class="form-controls">
										<input type="hidden" name="id" value="<?php echo $f['id']; ?>" />
										<input type="hidden" name="parent" value="<?php echo $f['parent']; ?>" />
										<input type="hidden" name="form_position" value="<?php echo $f['position']; ?>" />
										<input type="button" value="Remove" onclick="return removeFormElement($(this))" />
										<input type="button" value="Drag to Reorder" class="form-drag-handle" />
										<input type="button" value="Save" onclick="return saveFormElement($(this))" />
...
?>
						...
						<ul>
							<li class="form-item clone" style="display: none;">

									<label class="form-label">
										Name / Label<br />
										<input type="text" name="label" value="" class="width-100pct textfield" />
									</label>
...
?>
									...
									<div class="form-validation">
										...
										<label class="form-email">Does this field require a valid email address?<br />
											<select name="valid_email" class="width-100pct textfield">
												<option value="0">No</option>
												<option value="1">Yes</option>
											</select>
									...
									<label class="form-values">
										Drop Down / Checkbox / Radio Button Options<br />
										(Seperate each value with a comma. e.g. Example one,Sample Item 2,Three)<br />
										<input type="text" name="values" value="" class="width-100pct textfield" />
									</label>
						...
						<button class="save-order" onclick="return saveFormOrder();" >Save order</button>
