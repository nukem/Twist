<?php
$agents = dbq("SELECT `id`, `title` FROM `wp_structure` WHERE `type` = 'profile' AND online = 1");
...
if (isset($_POST['create_blog']))
{
	...
	if (count($blog_errors) < 1)
	{
		...
		else #insert
		{
			...
			header('Location: ?id=' . $cfg['blog']['id'] . '&action=edit&blog=' . $newid);
		}
	}
}
...
elseif(isset($_GET['action']) && $_GET['action'] == 'delete'){
	...
	header('Location: ?id=' . $cfg['blog']['id']);
}
elseif(isset($_GET['action']) && $_GET['action'] == 'comdelete'){
	...
	header('Location: ?id=' . $cfg['blog']['id'] . '&action=edit&blog=' . $_GET['blog']);
}
