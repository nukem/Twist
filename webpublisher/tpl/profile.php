          ...
          <div class="width-99pct"> 
            <table class="rec-table"> 
			  ...
			  <tr>
				<td colspan="1">
<?php
$clubs = dbq('SELECT * FROM wp_structure, wp_location WHERE id = link AND online = 1');
?>
					<label>Club</label><br />
					<select name="club[]" class="width-100pct textfield"  multiple="multiple">
						<option></option>
<?php
$curr_value = show_value('club');

$record['club'] = json_decode($record['club']);


foreach($clubs as $c) {
	$selected = '';
	if(!empty($curr_value) && ($c['id'] == show_value('club'))) {
		$selected = 'selected="selected"';
	}

?>
						<option value="<?php echo $c['id']; ?>" <?php if(is_array($record['club']) && in_array($c['id'], $record['club'])) echo "selected";?>><?php echo $c['title']; ?></option>
<?php
}
?>
					</select>
...
?>
              ...
              <tr> 
                <td colspan="4"> 
				<label>Club Information</label><br />
         <textarea name="club_content" cols="30" rows="10" class="textfield height-200 tinymce"><? if (isset ($_POST['club_content'])) echo htmlspecialchars ($_POST['club_content']); else echo htmlspecialchars (preg_replace('/src="/', 'src="../', $record['club_content'])); ?></textarea>
                </td> 
              </tr>
              <tr> 
                <td colspan="4"> 
				<label>Staff Profile</label><br />
         <textarea name="profile" cols="30" rows="10" class="textfield height-200 tinymce"><? if (isset ($_POST['profile'])) echo htmlspecialchars ($_POST['profile']); else echo htmlspecialchars (preg_replace('/src="/', 'src="../', $record['profile'])); ?></textarea>
                </td> 
              </tr>
              <tr> 
                <td colspan="4"> 
				<label>Achievements</label><br />
         <textarea name="skills" cols="30" rows="10" class="textfield height-200 tinymce"><? if (isset ($_POST['skills'])) echo htmlspecialchars ($_POST['skills']); else echo htmlspecialchars (preg_replace('/src="/', 'src="../', $record['skills'])); ?></textarea>
                </td> 
              </tr>
              <tr> 
                <td colspan="4"> 
				<label>Why I Love Taekwondo</label><br />
         <textarea name="tournaments" cols="30" rows="10" class="textfield height-200 tinymce"><? if (isset ($_POST['title'])) echo htmlspecialchars ($_POST['tournaments']); else echo htmlspecialchars (preg_replace('/src="/', 'src="../', $record['tournaments'])); ?></textarea>
                </td> 
              </tr>
			  <tr>
				<td colspan="2">
				...
				<td colspan="2">
				  <label>Upload Header Image (930 x 438px JPG)</label><br />
